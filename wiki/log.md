...
**Book change:** Added `#### When It's Been Months` inside "If You Realize You Fawned" (`src/concepts/before-you-facilitate.md`), between the five-step repair list and the section's closing paragraph. The section already handled the retroactive case in spirit ("It's late. It's still necessary.") but skipped the time dimension — and late is the most common case, since almost no facilitator catches their fawn in the moment. Structure per the plan's spec, in the item-D tiering pattern: **opening** shows the realization arriving months later and names the false test ("everyone has moved on"); **the mechanism** via the Context Rule cross-link — the room that never heard what actually happened moved on from the story that filled the silence, not from the truth, and non-correction is still lending the accusation validity ("Time doesn't dissolve that. It calcifies it. ... harm still being done, to the same person, every day it goes uncorrected"); **principle** — the vacuum is the ongoing harm, correct the record with whoever still carries the wrong one; **full version** — public correction to the same audience carrying the distorted story, "the correction should carry the same weight the silence did"; **minimum version** — targeted private corrections with the specific people who still matter to the harmed person's life ("reputational harm doesn't live in 'the group.' It lives in the specific relationships that changed"); **the gate** — ask the harmed person what they want first: an unrequested retroactive public correction is Rescuer behavior (cross-linked), reopening their wound to relieve your own conscience — they choose the repair's size and venue (cross-linked to `repair.md#ask-for-what-you-actually-want`). Voice guard honored: authoritative generosity, no self-doubting question marks. Privacy + link checks clean.

**Files updated:** `src/concepts/before-you-facilitate.md` (new subsection), `wiki/book-overview.md` (chapter bullet), `notes/fable-review-plan.md` (item O marked DONE), `wiki/log.md` (this entry).
