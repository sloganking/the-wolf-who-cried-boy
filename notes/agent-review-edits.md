...
## Added Aug 21, 2026 — Fable review item, from a live conversation (no names, per the standing privacy rule)

- [ ] **The book has no law-enforcement section, and the gap is real.** `handling-threats-of-violence.md` covers the facilitator/community response to a threat thoroughly (stop it, contain it, the promise, don't give the threatener power) — but it never tells the person who was *threatened* that a legal lane exists, or how to think about it. Consider adding a short section (there or in `when-youve-been-wronged.md`) covering **why filing a police report can be worth it even long after the event, even when prosecution is unlikely**:
  - **The core reason: a report on file makes the *next* victim more likely to be believed.** Police and prosecutors routinely check a suspect's prior contact history when a new report comes in; a documented pattern changes both charging decisions and credibility assessments. Two independent reports of the same behavior read completely differently than one.
  - **Partial legal grounding (verified against the statutes, worth a citation pass before print):** California Evidence Code **§1101(b)** admits prior acts to prove intent, plan, knowledge, or absence of mistake; **§1108/§1109** go further and admit prior acts as propensity evidence in sexual-offense and domestic-violence cases specifically; and course-of-conduct crimes (e.g. stalking, PC 646.9) are *built* out of documented prior incidents. So "the record helps the next case" is not folklore — it has statutory hooks, though the strength varies by offense type and the practical "more likely to be believed" claim is practice, not statute. **Mark the empirical strength of the practical claim as: investigate further / cite before print.**
  - **The reframe that belongs in the book's own voice:** filing late isn't revenge and isn't therapy — it's putting the record where records go, so the container that actually exists for proportionality (due process) holds the question instead of the harmed person carrying it privately forever. It pairs naturally with the responsibility chapter's bystander logic: the report protects people the writer will never meet.
  - **Also worth one honest paragraph:** reports mostly don't lead to charges, the reporter usually never hears back, and filing purely to feel better tends to disappoint — the record is the product, not the catharsis.
