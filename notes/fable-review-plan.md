...
**DONE 2026-07-03:** `#### When It's Been Months` added inside "If You Realize You Fawned," between the five-step repair list and the closing paragraph. **Kept (all four spec elements, in the spec's shape):** the principle (vacuum = ongoing harm, via the Context Rule cross-link and the perceived-validity logic already in the chapter — "your non-correction is still doing what it did on day one: lending the accusation validity by letting it stand"; "Time doesn't dissolve that. It calcifies it."); the full version (public correction to the same audience, "the correction should carry the same weight the silence did"); the minimum version (targeted private corrections — "reputational harm doesn't live in 'the group.' It lives in the specific relationships that changed"); the inarguable gate (ask the harmed person first — unrequested public correction is Rescuer behavior, cross-linked to `why-rescuers-are-dangerous.md` and `repair.md#ask-for-what-you-actually-want`; "they choose its size and its venue"). **Added beyond spec:** a show-first opening (the realization arriving months later, the false test "everyone has moved on" named and dismantled) so the tiers land on a felt problem rather than opening with prescription. **Dropped:** nothing from the spec. Voice: authoritative generosity throughout; no question marks. Privacy + link checks clean; wiki `book-overview.md` bullet updated; log entry appended.
