...
- **Became:** "### Step 1: Get the Attacked Person to Safety First" — the goal, not the mechanism, and not the punishment verb. Glossary Order of Operations now reads *get the attacked person to safety → contain violent person → remove violent person → address crowd*, so the distinction is visible in four words. The disclaimer was **cut outright**: with the verb fixed, the section doesn't need a defense, and the room's misreading is already stated properly two sections later in The Dangerous Gap ("People are watching. They saw you remove the attacked person and _not_ the person making death threats"), which has a named subject and an observable basis.
- **The intermediate miss, worth recording.** The first replacement disclaimer was "It will look like punishment. It isn't." Author caught it as **Don't Pre-Frame the Reception**, and he was right: the sentence never names *who* does the looking, so it defaults to the reader — a forecast about the facilitator's own reception, one line before the instruction. It also states a prediction as certainty ("it will"). His proposed fix ("some people, at first glance, might think this is punishment") repairs the pre-frame but keeps a disclaimer the section no longer needs and introduces a claim about other people's inner states — so the line went entirely. Note the sequence: the fix for a told correction was written *as another told correction*. Changing the word and keeping the defense is the default failure; the defense is the thing to delete.
